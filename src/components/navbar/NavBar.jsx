        ...
        >
          ...
          >
            ...
            {navItems.map((item) => (
              <li
                key={item.id}
                className="md:m-6 md:flex md:gap-6 md:items-center md:justify-center hover:scale-125"
              >
                <a
                  onClick={() => toggleNav(item.name)}
                  href={`#${item.name}`}
                  className={`uppercase cursor-pointer text-black hover:text-indigo-400 hover:scale-125 font-bold ${
                    item.name === activeIndex ? "text-indigo-400 " : ""
                  }`}
                >
                  {item.name}
                </a>
            ...
            <div >
              <a href={"https://wa.me/7990977945"} target="blank">
              <img
